...
Version 7.3.0 brings improved transcoding, additional type annotations,
automatic subclassing on domain entities of domain events (not enabled
by default), support for saving and deleting custom ORM objects from a
process application policy (so that they will be stored atomically with
position of the processed event notification), an option to apply the policy
of a process application to all events that are generated by its policy
when an event notification is processed (continues until all successively
generated events have been processed, with all generated events stored in
the same atomic process event, as if all generated events were generated
in a single policy function).
...
of this library will continue to be returned as lists, since they were encoded
as lists not tuples.
