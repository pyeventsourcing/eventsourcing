...
The three important aspects of any :class:`~eventsourcing.dcb.domain.Slice` are:
...
:data:`~eventsourcing.dcb.domain.Perspective.consistency_boundary`, using some of the constructor
arguments used to construct :ref:`selectors <DCB Selector>`.

We can define the **projection** of selected :ref:`decisions <DCB decision>` with the
:func:`@event <eventsourcing.domain.event>` decorator, using other constructor arguments
to initialise the projected state.

We can define the **action** of the slice, if it has one, by implementing an :func:`~eventsourcing.dcb.domain.Slice.execute`
method. New events generated by the :func:`~eventsourcing.dcb.domain.Slice.execute` method can be accumulated
with the :func:`~eventsourcing.dcb.domain.Perspective.append_new_decision` method.
