...
The type variable :type:`~eventsourcing.domain.TDecision` represents
any subclass of :class:`~eventsourcing.domain.Decision`. It will be used
...
The generic class :class:`~eventsourcing.domain.Event` encapsulates a
:class:`~eventsourcing.domain.Decision`, along with some tag strings,
...
:class:`~eventsourcing.domain.Event` objects. The
:data:`~eventsourcing.domain.Event.uuid` attribute is initialised by
default with a version 4 UUID. The :data:`~eventsourcing.domain.Event.metadata`
...
of the decision class topic, and the straightforward transfer of the :data:`~eventsourcing.domain.Event.uuid`
and :data:`~eventsourcing.domain.Event.metadata` attributes.
...
A :class:`~eventsourcing.domain.Selector` defines a criterion for a consistency boundary
...
.. code-block:: python
    ...
    from eventsourcing.domain import Selector
...
The :class:`~eventsourcing.domain.Perspective` class is an abstract base class for different
kinds of decision models. It defines an abstract method :func:`~eventsourcing.domain.Perspective.consistency_boundary`
that must be implemented by subclasses to return a :ref:`selector <DCB Selector>` or a sequence of selectors. It also
defines a :data:`~eventsourcing.domain.Perspective.last_known_position` attribute for keeping track of the last
...
The :class:`~eventsourcing.domain.Perspective` class also provides
:func:`~eventsourcing.domain.Perspective.trigger_event` for generating and applying and appending new events
to an internal list of pending events, and :func:`~eventsourcing.domain.Perspective.collect_events` for collecting
all newly generated events.

.. code-block:: python

    from eventsourcing.domain import Perspective
...
The generic base class :class:`~eventsourcing.domain.EnduringObject` extends the :ref:`perspective <Perspective>` class,
and is similar to :doc:`event-sourced aggregates </topics/tutorial/part2>`. Each instance has a unique continuity ID,
which should be stored in the :data:`~eventsourcing.domain.EnduringObject.id` attribute. The continuity ID is used as a tag
...
The generated events can be collected by calling :func:`~eventsourcing.domain.Perspective.collect_events`.
...
The :class:`~eventsourcing.domain.Group` class extends the :ref:`perspective <Perspective>` class, and supports
...
using the group's :func:`~eventsourcing.domain.Group.trigger_event` method. An event generated by
...
.. code-block:: python
    ...
    from eventsourcing.domain import Group
...
* **Consistency Boundary** — implement :func:`~eventsourcing.domain.Perspective.consistency_boundary` to return :ref:`selectors <DCB Selector>`.

* **Projection** — use the :ref:`event decorator <Event decorator>` to define how selected :ref:`decisions <DCB decision>` evolve state.

* **Command Action** — implement :func:`~eventsourcing.domain.Slice.execute` to generate new :ref:`decisions <DCB decision>`.
...
The example below shows a slice for updating a student's name. The :func:`~eventsourcing.domain.Perspective.consistency_boundary`
...
.. code-block:: python
    ...
    from eventsourcing.domain import Slice
