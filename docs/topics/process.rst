...
without causing anything to need retrying.

Responses can be collected on the command aggregate (see below), in which it will
need to follow another application. An alternative approach to updating the
command involves creating separate "command response" aggregates in a "responses"
application. Another advantage of having distinct commands is that old commands
can be used to check the same application state is generated by a new version
of the system.
...
The concrete application infrastructure class is
:class:`~eventsourcing.application.popo.PopoApplication`
which uses Plain Old Python Objects to store domain events,
and is the fastest application infrastructure class in the
library. It can be used when proper disk-based durability
is not required, for example when testing.
...
For convenience in the examples below, let's redefine ``system`` to use an
infrastructure class by default. The ``infrastructure_class`` is set to
:class:`~eventsourcing.application.sqlalchemy.SQLAlchemyApplication`,
which means SQLAlchemy will be used to store data.

It's possible to override this value by passing an application
infrastructure class to system runners, but setting a default
