...
It implements :func:`~eventsourcing.projection.Projection.process_event` by calling ``incr_created_event_counter()``
for :class:`Aggregate.Created <eventsourcing.domain.Aggregate.Created>` events and ``incr_subsequent_event_counter()``
for :class:`Aggregate.Event <eventsourcing.domain.Aggregate.Event>` events.
