...
class PopoRecordManager(ACIDRecordManager):
    ...
    def delete_record(self, record):
        ...
        try:
            # This isn't really what happens in ACID DB, but hey.
            all_sequence_ids[record.position] = None
        except IndexError:
            pass
    ...
    def get_records(self, sequence_id, gt=None, gte=None, lt=None, lte=None, limit=None,
        ...
        selected_records = all_sequence_records[start:end]

        # Overlook "deleted" records (set to None).
    ...
    def _record_sequenced_item(self, sequenced_item):
        ...
        if sequenced_item.position != len(all_records):
            raise RecordConflictError(sequenced_item.position, len(all_records))
        all_records.append(sequenced_item)
