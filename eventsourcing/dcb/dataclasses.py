...
if TYPE_CHECKING:
    from eventsourcing.persistence import Cipher, Compressor

_T = TypeVar("_T", bound="MetaDecision")


class MetaDecision(ABCMeta):
    def __call__(cls: MetaDecision, **kwargs: Any) -> Any:
        validated_kwargs = {}
    ...
    def __new__(
        ...
        event_cls = dataclasses.dataclass(kw_only=True)(event_cls)
...
class Decision(eventsourcing.dcb.domain.Decision, metaclass=MetaDecision):
    # class Decision(eventsourcing.dcb.domain.Decision):
...
class DataclassMapper(DCBMapper[TDecision]):
    def __init__(
        self,
        compressor: Compressor | None = None,
        cipher: Cipher | None = None,
    ):
        super().__init__(compressor, cipher)
    ...
    def _to_decision(self, data: bytes, decision_class: type[TDecision]) -> TDecision:
        ...
        return decision_class(**data_dict)
