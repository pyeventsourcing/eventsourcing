...
from eventsourcing.interface.archived_logs import ArchivedLogReader, ArchivedLogRepo, RemoteArchivedLogRepo, \
    deserialise_archived_log, serialize_archived_log
...
class ArchivedLogTestCase(WithSQLAlchemyActiveRecordStrategies, WithPersistencePolicies):
    ...
    def create_big_array_repo(self, big_array_id=None):
    ...
    def assert_archived_log(self, repo, archived_log_id, expected_id, expected_len_items, expected_previous_id,
        ...
        self.assertEqual(len(list(archived_log.items)), expected_len_items)
...
class Application(object):
    ...
    def create_archived_log_repo(self, big_array, doc_size):
        return ArchivedLogRepo(big_array, doc_size=doc_size)


class TestArchivedLog(ArchivedLogTestCase):

    def test_archived_log_repo(self):
        # Build archived log repo.
        big_array = self.create_big_array_repo()
        doc_size = 5
        archived_log_repo = self.app.create_archived_log_repo(big_array, doc_size=doc_size)

        # Check the archived logs.
        archived_log = archived_log_repo['current']
        self.assertEqual(archived_log.id, '1,5')
        self.assertEqual(len(list(archived_log.items)), 0)
        ...
        # Check the archived logs.
        self.assert_archived_log(archived_log_repo, 'current', '11,15', 3, '6,10', None)
        self.assert_archived_log(archived_log_repo, '1,5', '1,5', doc_size, None, '6,10')
        self.assert_archived_log(archived_log_repo, '6,10', '6,10', doc_size, '1,5', '11,15')
        self.assert_archived_log(archived_log_repo, '11,15', '11,15', 3, '6,10', None)
        self.assert_archived_log(archived_log_repo, '16,20', '16,20', 0, '11,15', None)
        self.assert_archived_log(archived_log_repo, '21,25', '21,25', 0, '16,20', None)

        # Add some more items.
        self.append_notifications(big_array, 13, 24)

        # Check the archived logs have been extended.
        self.assertEqual(len(list(archived_log_repo['11,15'].items)), doc_size)
        self.assertEqual(len(list(archived_log_repo['16,20'].items)), doc_size)
        self.assertEqual(len(list(archived_log_repo['21,25'].items)), 4)
        self.assertEqual(len(list(archived_log_repo['26,30'].items)), 0)

        # Check sequence size must be divisible by doc size.
        with self.assertRaises(ValueError):
            self.app.create_archived_log_repo(big_array, doc_size=6)

        # Check the doc ID must match the doc size.
        archived_log_repo = self.app.create_archived_log_repo(big_array, doc_size)
        with self.assertRaises(ValueError):
            _ = archived_log_repo['1,2']

        # Check the doc ID must be aligned to the doc size.
        archived_log_repo = self.app.create_archived_log_repo(big_array, doc_size)
        with self.assertRaises(ValueError):
            _ = archived_log_repo['2,6']

        # Check an archived log ID that can't be split by ',' results in a ValueError.
        with self.assertRaises(ValueError):
            _ = archived_log_repo['invalid']
...
class TestArchivedLogWithCassandra(WithCassandraActiveRecordStrategies, TestArchivedLog):
    pass
