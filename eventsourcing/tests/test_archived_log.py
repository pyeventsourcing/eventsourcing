...
from eventsourcing.interface.archived_logs import ArchivedLogRepo, deserialise_archived_log
...
class TestArchivedLog(WithSQLAlchemyActiveRecordStrategies, WithPersistencePolicies):
    def setUp(self):
        super(TestArchivedLog, self).setUp()
    ...
    def test_archived_log_repo(self):
        ...
        doc_size = 5
        archive_log_repo = self.app.create_archived_log_repo(big_array, doc_size=5)

        # Check the archived logs.
        archived_log = archive_log_repo['current']
        ...
        # Check the archived logs.
        self.assert_archived_log(archive_log_repo, 'current', '11,15', 3, '6,10', None)
        self.assert_archived_log(archive_log_repo, '1,5', '1,5', doc_size, None, '6,10')
        self.assert_archived_log(archive_log_repo, '6,10', '6,10', doc_size, '1,5', '11,15')
        self.assert_archived_log(archive_log_repo, '11,15', '11,15', 3, '6,10', None)
        self.assert_archived_log(archive_log_repo, '16,20', '16,20', 0, '11,15', None)
        self.assert_archived_log(archive_log_repo, '21,25', '21,25', 0, '16,20', None)

        # Add some more items.
        self.append_notifications(big_array, 13, 24)

        # Check the archived logs have been extended.
        self.assertEqual(len(list(archive_log_repo['11,15'].items)), doc_size)
        self.assertEqual(len(list(archive_log_repo['16,20'].items)), doc_size)
        self.assertEqual(len(list(archive_log_repo['21,25'].items)), 4)
        self.assertEqual(len(list(archive_log_repo['26,30'].items)), 0)
        ...
        archive_log_repo = self.app.create_archived_log_repo(big_array, doc_size)
        with self.assertRaises(ValueError):
            _ = archive_log_repo['1,2']

        # Check the doc ID must be aligned to the doc size.
        archive_log_repo = self.app.create_archived_log_repo(big_array, doc_size)
        with self.assertRaises(ValueError):
            _ = archive_log_repo['2,6']

        # Check an archived log ID that can't be split by ',' results in a ValueError.
        with self.assertRaises(ValueError):
            _ = archive_log_repo['invalid']
