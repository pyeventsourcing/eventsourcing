...
from eventsourcing.infrastructure.stored_events.base import StoredEventIterator
...
class TestStoredEvent(unittest.TestCase):
    ...
    def test_serialize_domain_event(self):
        ...
        event1 = DomainEvent(a=1, b=2, c=datetime_now, d=datetime_now_tzaware, e=date_now, entity_version=0, entity_id='entity1', uuid=3)
        ...
        self.assertEqual('{"a":1,"b":2,"c":{"ISO8601_datetime":"2015-09-08T16:20:50.577429"},"d":{"ISO8601_datetime":"2015-09-08T16:20:50.577429+0000"},"e":{"ISO8601_date":"2015-09-08"},"entity_id":"entity1","entity_version":0,"uuid":3}',
    ...
    def test_serialize_domain_event_with_numpy_array(self):
        ...
        if numpy is not None:
            event1 = DomainEvent(a=numpy.array([10.123456]), entity_version=0, entity_id='entity1', uuid=3)

            stored_event = serialize_domain_event(event1, json_encoder_cls=ObjectJSONEncoder)
            self.assertEqual('eventsourcing.domain.model.events#DomainEvent', stored_event.event_topic)
            self.assertEqual('{"a":{"__ndarray__":"\\"\\\\u0093NUMPY\\\\u0001\\\\u0000F\\\\u0000{\'descr\': \'<f8\', \'fortran_order\': False, \'shape\': (1,), }            \\\\nm\\\\u00fd\\\\u00f4\\\\u009f5?$@\\""},"entity_id":"entity1","entity_version":0,"uuid":3}',
                             stored_event.event_attrs)
        else:
    ...
    def test_recreate_domain_event(self):
        stored_event = StoredEvent(event_id='1',
                                   ...
                                   event_attrs='{"a":1,"b":2,"c":{"ISO8601_datetime":"2015-09-08T16:20:50.577429"},"d":{"ISO8601_datetime":"2015-09-08T16:20:50.577429+0000"},"e":{"ISO8601_date":"2015-09-08"},"entity_id":"entity1","entity_version":0,"uuid":3}')
        ...
        self.assertEqual(3, domain_event.uuid)
...
class StoredEventRepositoryTestCase(unittest.TestCase):
    ...
    def checkStoredEventRepository(self, stored_event_repo):
        ...
        self.assertEqual(2, len(retrieved_events))
        ...
        # retrieved_events = stored_event_repo.get_topic_events('eventsourcing.domain.model.example#Example.Created')
        # retrieved_events = list(retrieved_events)
        # self.assertEqual(2, len(retrieved_events))

        # - check the last event is first in the list
        # self.assertIsInstance(retrieved_events[0], StoredEvent)
        # self.assertEqual(stored_event2.event_topic, retrieved_events[0].event_topic)
        # self.assertEqual(stored_event2.event_attrs, retrieved_events[0].event_attrs)
        #
        # # - check the first event is last in the list
        # self.assertIsInstance(retrieved_events[1], StoredEvent)
        # self.assertEqual(stored_event1.event_topic, retrieved_events[1].event_topic)
